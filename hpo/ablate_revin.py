"""RevIN ablation: compare RevIN=True vs RevIN=False.

Tokenier and BaseModel backbone fixed. Only varies use_revin.
Saves results under trials/ablate_revin/.

Usage:
    python -m hpo.ablate_revin
"""
...
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "trials", "ablate_revin")
...
BASEMODEL = {
    ...
    "batch_size": 16, "accumulation_steps": 2, "grad_clip": 0.3,
...
def _run_downstream_eval(model, tokenizer, device):
    ...
    for start in tqdm(range(0, N, EVAL_BATCH_SIZE), desc="  Eval 1-step", leave=False):
        ...
        t_year = tf["year"][idx][:, :1023].to(device, non_blocking=True).long()
        with _autocast_ctx(use_amp, amp_dtype):
            lc, lf, _ = model(ic, i_f, t_min, t_day, t_mon, t_year, last_only=True)
...
def _load_phase3_baseline() -> dict:
    """RevIN=ON baseline from Phase 3 trial 047 (same backbone)."""
    p3_path = os.path.join(PROJECT_ROOT, "trials", "phase3_basemodel", "trial_047", "result.json")
    if os.path.exists(p3_path):
        with open(p3_path) as f:
            r = json.load(f)
        ds = r.get("downstream", {})
        return {"use_revin": True, "best_val_ce": r["best_val_ce"],
                "epoch_stopped": r["epoch_stopped"],
                "downstream": ds, "source": "phase3_trial_047"}
    raise FileNotFoundError(f"Phase 3 baseline not found: {p3_path}")


def run_one(use_revin: bool, device: torch.device) -> dict:
    # RevIN=ON already covered by Phase 3 trial 047
    if use_revin:
        print("  RevIN=ON: loading from Phase 3 trial 047...")
        return _load_phase3_baseline()

    bp = {**BASEMODEL, "use_revin": use_revin}
    vocab = 1 << TOKENIZER_BITS
    label = "RevIN=OFF"
    tdir = os.path.join(OUTPUT_DIR, "revin_off")
    ...
    patience_counter = 0
    stopped_epoch = bp["max_epochs"]

    for epoch in range(bp["max_epochs"]):
        model.train(); opt.zero_grad(set_to_none=True)
        tl, bd = 0.0, 0
        for bi, batch_data in enumerate(
            tqdm(train_loader, desc=f"  {label} {epoch+1}/{bp['max_epochs']}", leave=False), start=1
        ):
        ...
        print(f"  {label} epoch {epoch+1:2d}  val_ce={avg_val:.4f}  best={best_val_ce:.4f}  patience={patience_counter}/{bp['early_stop_patience']}")
        if patience_counter >= bp["early_stop_patience"]: stopped_epoch = epoch + 1; break

    eval_metrics = _run_downstream_eval(model, tokenizer, device)
    result = {"use_revin": use_revin, "best_val_ce": round(best_val_ce, 6),
              "epoch_stopped": stopped_epoch, "downstream": eval_metrics}
    with open(result_path, "w") as f: json.dump(result, f, indent=2)
    print(f"  {label}: MAPE={eval_metrics['mape']:.4f}%  DA={eval_metrics['da']:.2f}%  val_ce={best_val_ce:.4f}")
    return result


def main():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"RevIN Ablation")
    ...
    r_on  = run_one(use_revin=True,  device=device)  # loaded from Phase 3 trial 047
    r_off = run_one(use_revin=False, device=device)

    print(f"\n{'='*50}")
    print(f"RevIN Ablation Results")
    print(f"{'='*50}")
    on_src = r_on.get("source", "?")
    print(f"  RevIN=ON:   MAPE={r_on['downstream']['mape']:.4f}%  DA={r_on['downstream']['da']:.2f}%  val_ce={r_on['best_val_ce']:.4f}  (from {on_src})")
    print(f"  RevIN=OFF:  MAPE={r_off['downstream']['mape']:.4f}%  DA={r_off['downstream']['da']:.2f}%  val_ce={r_off['best_val_ce']:.4f}")
    winner = "ON" if r_on['downstream']['mape'] < r_off['downstream']['mape'] else "OFF"
    delta = abs(r_on['downstream']['mape'] - r_off['downstream']['mape'])
    print(f"  Winner: RevIN={winner}  (delta={delta:.4f}pp)")
